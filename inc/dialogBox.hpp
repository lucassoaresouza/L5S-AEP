...
class DialogBox : public Engine::Field {
 ...
 public:
  DialogBox(std::string name, std::pair<int, int> position,
            std::pair<int, int> size);
  ~DialogBox() {};
  bool is_active();
  void activate();
  void deactivate();
  void set_icon(std::string path, std::pair<int, int> obj_position,
                std::pair<int, int> size);
  void set_text(std::string text, std::pair<int, int> obj_position);
  bool load();
  void draw();
};
