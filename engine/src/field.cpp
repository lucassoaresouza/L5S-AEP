...
Field::Field(std::string object_name, std::pair<int, int> object_position,
...
void Field::draw_texts() {
  ...
  for (int i = 0; i < 10; i++) {
    if (font_texture[i]) {
      SDL_RenderCopy(game.get_renderer(), font_texture[i], NULL,
                     &text_quads[i]);
    }
  }
}

void Field::set_bold(bool bold) { is_bold = bold; }
...
void Field::set_text_per_line(std::string new_text, int line) {
  ...
  if (text[line] != "") {
    ...
    font_surface =
        TTF_RenderText_Blended(font, text[line].c_str(), black_font_color);
    text_quads[line] = {position.first + 15,
                        position.second + 15 + (font_surface->h * line),
                        font_surface->w, font_surface->h};
    SDL_Texture* aux_texture =
        SDL_CreateTextureFromSurface(game.get_renderer(), font_surface);
