...
Button::Button(std::string object_name, std::pair<int, int> object_position,
...
void Button::set_sprites(std::string sprite_not_pressed,
                         std::string sprite_pressed) {
  set_sprite(sprite_not_pressed);
  pressed_sprite = sprite_pressed;
}

void Button::activate() { is_active = true; }

void Button::deactivate() { is_active = false; }
...
bool Button::load() {
  ...
  SDL_Surface* provisory_not_pressed_surface =
      IMG_Load(not_pressed_sprite.c_str());
  SDL_Surface* provisory_pressed_surface = IMG_Load(pressed_sprite.c_str());

  if (provisory_not_pressed_surface == NULL ||
    ...
    in_load_not_pressed_texture = SDL_CreateTextureFromSurface(
        game.get_renderer(), provisory_not_pressed_surface);
    in_load_pressed_texture = SDL_CreateTextureFromSurface(
        game.get_renderer(), provisory_pressed_surface);
    if (in_load_not_pressed_texture == NULL ||
...
bool Button::get_pressed() { return pressed; }