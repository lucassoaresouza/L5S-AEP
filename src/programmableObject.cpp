...
ProgrammableObject::ProgrammableObject(std::string object_name,
...
void ProgrammableObject::add_commands(command_list new_commands) {
...
void ProgrammableObject::draw() {
  ...
  SDL_RenderCopyEx(game.get_renderer(), texture, NULL, &ret, direction_angle,
                   NULL, flip);
  execute();
}
...
void ProgrammableObject::move(int distance, int displacement) {
  ...
  if (position.first >= expected_x || position.second >= expected_y) {
    command_index += 1;
    expected_x = 0;
    expected_y = 0;
  }
...
void ProgrammableObject::execute() {
  if (command_index < commands.size() && running == true) {
    ...
    if (command_index == commands.size() && command_index != 0) {
...
void ProgrammableObject::set_limits(std::pair<int, int> min,
...
bool ProgrammableObject::verify_limits() {
  if (position.first < limits.first.first ||
...
std::string ProgrammableObject::get_status() { return status; }
