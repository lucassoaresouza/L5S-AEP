...
SelectRobotMenu::SelectRobotMenu(std::string screen_name) {
...
void SelectRobotMenu::init_background() {
  Field* box_field =
      new Field("box_field", box_position, std::make_pair(455, 725));
  box_field->set_color(219, 166, 80, 0);
  add_object(box_field);

  Field* background_field = new Field("selector_field", background_position,
                                      std::make_pair(390, 258));
  background_field->set_color(179, 102, 0, 0);
  add_object(background_field);

  Field* background_field_2 =
      new Field("challenge_selector_field", background_position_2,
...
void SelectRobotMenu::init_selector() {
  selector_field = new Selector("selector_field", selector_field_position,
                                std::make_pair(70, 70), 2, 3, 100);
  selector_field->set_sprite("./assets/sprites/tiles/border.png");
...
void SelectRobotMenu::init_robots() {
  ...
  for (int i = 97; i < 103; i++) {
    ...
    Field* aux_field =
        new Field("robot_" + i, aux_robot_position, std::make_pair(64, 64));
    aux_field->set_sprite(robot_base_path + aux_letter + ".png", "");
...
void SelectRobotMenu::init_init_button() {
  ...
  init_challenge_button =
      new InitButton("init_challenge_button", button_position, button_size);
  init_challenge_button->set_sprites("./assets/sprites/buttons/button1.png",
...
void SelectRobotMenu::init_texts() {
  Engine::Field* description = new Engine::Field(
      "title-robts", description_position, std::make_pair(0, 0));
  ...
  Engine::Field* operator_name = new Engine::Field(
      "username", operator_name_position, std::make_pair(0, 0));
...
void SelectRobotMenu::init_select_challenge_objects() {
  Engine::Field* description_2 = new Engine::Field(
      "description-2", description_2_position, std::make_pair(0, 0));
  description_2->set_bold(true);
  description_2->set_font("./assets/fonts/larabiefont-rg.ttf", 15);
  description_2->set_text_per_line("Agora, por gentileza, selecione o desafio:",
                                   0);
  add_object(description_2);

  std::pair<int, int> button_size(40, 40);
  next_challenge_button = new Engine::Button(
      "next_challenge_button", next_challenge_button_position, button_size);
  next_challenge_button->set_sprites("./assets/sprites/buttons/next1.png",
  ...
  challenge_title = new Engine::Field(
      "challenge_title", challenge_title_position, std::make_pair(0, 0));
  ...
  challenge_title->set_text_per_line(
      challenge_creator->get_challenge_title(challenge_index), 0);
  add_object(challenge_title);

  challenge_progress = new Engine::Field(
      "challenge-progress", challenge_progress_position, std::make_pair(0, 0));
  ...
  back_challenge_button = new Engine::Button(
      "back_challenge_button", back_challenge_button_position, button_size);
  back_challenge_button->set_sprites("./assets/sprites/buttons/back1.png",
...
void SelectRobotMenu::select_challenge() {
  ...
  if (challenge_creator && current_user) {
    if (next_challenge_button->get_pressed() &&
        challenge_index < challenge_creator->get_challenge_count() - 1) {
      next_aux++;
    } else if (back_challenge_button->get_pressed() && challenge_index > 0) {
      back_aux++;
    }
...
void SelectRobotMenu::update_challenge_info() {
  challenge_name = (challenge_creator->get_challenge_title(challenge_index));

  challenge_title->set_text_per_line(challenge_name, 0);
  int user_progress = (current_user->get_challenge_progress(challenge_name));

  challenge_progress->set_text_per_line(
      "realizados: " + std::to_string(user_progress) + " | total: " +
          std::to_string(
              challenge_creator->get_challenge_maps_count(challenge_name)),
      0);
  if (init_challenge_button) {
    init_challenge_button->set_challenge_info(challenge_name, user_progress);
  }
}