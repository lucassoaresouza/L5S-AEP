...
void ChallengeCreator::read_folder() {
  ...
  if (dir != NULL) {
    ...
    while ((lsdir = readdir(dir)) != NULL) {
      ...
      if (file_std_string.substr(file_std_string.find_last_of(".") + 1) ==
          "aep") {
    ...
    if (challenge_file.is_open()) {
      while (getline(challenge_file, line)) {
        if (line == "init") {
          ...
          ChallengeMap* new_map = new ChallengeMap(name, position, map, text);
          new_map->set_obj_initial_position(stoi(obj_x), stoi(obj_y));
...
int ChallengeCreator::get_challenge_count() { return challenges.size(); }

int ChallengeCreator::get_challenge_maps_count(std::string name) {
  return (get_challenge_by_title(name)->get_maps_count());
}