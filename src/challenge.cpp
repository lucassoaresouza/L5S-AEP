...
Challenge::Challenge(std::string challenge_title) { title = challenge_title; }
...
void Challenge::add_map(ChallengeMap* map) { maps.push_back(map); }

std::string Challenge::get_title() { return title; }
...
int Challenge::get_maps_count() { return maps.size(); }