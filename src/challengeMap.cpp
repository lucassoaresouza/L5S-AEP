...
ChallengeMap::ChallengeMap(std::string map_name, std::pair<int, int> position,
...
void ChallengeMap::load_map_info() {
  ...
  std::pair<int, int> aux_size(tile_quad_size, tile_quad_size);
  ...
  for (int i = 0; i < map_info.size(); i++) {
    ...
    aux_position.second = (position.second + index_line * size_with_spacing);
    aux_field = new CheckableField(tile_name, aux_position, aux_size);
    switch (map_info[i]) {
      case 'G':
...
void ChallengeMap::add_background() {
  ...
  columns += 1;
  lines += 1;
  std::pair<int, int> background_size((columns * size_with_spacing) + spacing,
                                      (lines * size_with_spacing) + spacing);

  std::pair<int, int> background_position(position.first - spacing,
                                          position.second - spacing);

  CheckableField* background_field = new CheckableField(
      "background_field", background_position, background_size);
...
void ChallengeMap::line_border() {
  ...
  for (int i = 0; i < lines; i++) {
    ...
    CheckableField* border_line_field =
        new CheckableField("border_field", border_position,
...
void ChallengeMap::column_border() {
  ...
  for (int i = 0; i < columns - 1; i++) {
    ...
    CheckableField* border_column_field =
        new CheckableField("border_field", border_position,
                           std::pair<int, int>(tile_quad_size, tile_quad_size));
...
std::pair<std::pair<int, int>, std::pair<int, int>> ChallengeMap::get_limits() {
  ...
  limits.first =
      std::make_pair(position.first + tile_quad_size, position.second);
  limits.second =
      std::make_pair(position.first + (tile_quad_size * columns),
                     position.second + (tile_quad_size * (lines - 1)));
  return limits;
}

void ChallengeMap::set_completed(bool status) { completed = status; }

bool ChallengeMap::get_completed() { return completed; }
...
std::pair<int, int> ChallengeMap::get_obj_initial_position() {
  return get_possible_position(obj_initial_position.first,
...
std::string ChallengeMap::get_text_info() { return text_info; }
